    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, @Nonnull EnumHand hand) {
        ...
        if (stack.getItemDamage() != 0 || hand != EnumHand.MAIN_HAND) {
            ...
            if (lives >= QuestSettings.INSTANCE.getProperty(NativeProps.LIVES_MAX)) {
                if (!world.isRemote) {
                    player.sendStatusMessage(new TextComponentString(TextFormatting.RED.toString()).appendSibling(new TextComponentTranslation(
                                                                                                                                               "betterquesting.gui.full_lives")),
                                             true);
                }
            ...
            if (!world.isRemote) {
                ...
                player.sendStatusMessage(new TextComponentTranslation("betterquesting.gui.remaining_lives", TextFormatting.YELLOW.toString() + (lives + 1)),
                                         true);
            }
