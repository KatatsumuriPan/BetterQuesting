...
public class GuiStatus extends GuiScreenCanvas {
    ...
    public void initPanel() {
        ...
        CanvasHoverTray cvHover = new CanvasHoverTray(new GuiTransform(GuiAlign.MID_CENTER, 16, 16, 16, 16, 0),
                new GuiTransform(GuiAlign.MID_CENTER, 16, 16, 64, 64, 5), PresetTexture.PANEL_DARK.getTexture());
        ...
        CanvasTextured bgSideBar = new CanvasTextured(
                new GuiTransform(new Vector4f(0F, 0F, 0.2F, 1F), new GuiPadding(8, 24, 4, 24), 0),
                PresetTexture.PANEL_INNER.getTexture());
        bgCan.addPanel(bgSideBar);

        CanvasScrolling cvBtnList = new CanvasScrolling(
                new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(0, 0, 8, 0), 0));
        bgSideBar.addPanel(cvBtnList);

        PanelVScrollBar scBtnList = new PanelVScrollBar(
        ...
        CanvasTextured bgQuickBar = new CanvasTextured(
                new GuiTransform(new Vector4f(0F, 1F, 0.2F, 1F), new GuiPadding(8, -24, 4, 8), 0),
                PresetTexture.PANEL_INNER.getTexture());
        bgCan.addPanel(bgQuickBar);

        PanelButton btnSettings = new PanelButton(
                new GuiTransform(new Vector4f(0F, 0F, 0.25F, 1F), new GuiPadding(0, 0, 0, 0), 0), 0, "");
        btnSettings.setIcon(PresetIcon.ICON_GEAR.getTexture());
        bgQuickBar.addPanel(btnSettings);

        PanelButton btnParty = new PanelButton(
                new GuiTransform(new Vector4f(0.25F, 0F, 0.5F, 1F), new GuiPadding(0, 0, 0, 0), 0), 0, "");
        btnParty.setIcon(PresetIcon.ICON_PARTY.getTexture());
        bgQuickBar.addPanel(btnParty);

        PanelButton btnInbox = new PanelButton(
                new GuiTransform(new Vector4f(0.5F, 0F, 0.75F, 1F), new GuiPadding(0, 0, 0, 0), 0), 0, "");
        btnInbox.setIcon(PresetIcon.ICON_NOTICE.getTexture());
        bgQuickBar.addPanel(btnInbox);

        PanelButton btnTheme = new PanelButton(
                new GuiTransform(new Vector4f(0.75F, 0F, 1F, 1F), new GuiPadding(0, 0, 0, 0), 0), 0, "");
        btnTheme.setIcon(PresetIcon.ICON_THEME.getTexture());
        bgQuickBar.addPanel(btnTheme);

        // === OUTER CHARACTER PANEL ===

        // CanvasTextured bgChar = new CanvasTextured(new GuiTransform(new Vector4f(0.2F, 0F, 1F, 1F), new GuiPadding(4,
        // 24, 8, 48), 0), PresetTexture.PANEL_INNER.getTexture());
        // bgCan.addPanel(bgChar);

        // === CHARACTER PREVIEW ===

        // PanelEntityPreview pvChar = new PanelEntityPreview(new GuiTransform(GuiAlign.HALF_LEFT, new GuiPadding(0, 0,
        // 4, 8), 0), mc.player);
