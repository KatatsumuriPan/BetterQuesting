...
public class GuiItemSelection extends GuiScreenCanvas implements IPEventListener, IVolatileScreen {
    ...
    public void initPanel() {
        ...
        itemPreview = new PanelItemSlot(new GuiTransform(GuiAlign.TOP_LEFT, 0, 16, 36, 36, 0), 99, itemStack, false,
                true);
        ...
        String oreName = "NONE";
        int oreIdx = -1;
        if (itemStack != null && !itemStack.getBaseStack().isEmpty() &&
                !StringUtils.isNullOrEmpty(itemStack.getOreDict())) {
        ...
        btnOre = new PanelButtonStorage<>(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(52, 36, 24, -52), 0), 2,
                "OreDict: " + oreName, oreIdx);
        ...
        for (int i = 0; i < 27; i++) // Main inventory
        {
            ...
            cvBottomLeft.addPanel(new PanelItemSlot(new GuiTransform(GuiAlign.TOP_LEFT, x, y, slotSize, slotSize, 0), 1,
                    invoStack, true).setCallback(c -> {}));
        ...
        for (int i = 0; i < 9; i++) // Hotbar
        {
            ...
            cvBottomLeft.addPanel(new PanelItemSlot(
                    new GuiTransform(GuiAlign.TOP_LEFT, x, 20 + (3 * slotSize), slotSize, slotSize, 0), 1, invoStack,
                    true).setCallback(c -> {}));
        }
    ...
    private void onButtonPress(PEventButton event) {
        ...
        if (btn.getButtonID() == 0) // Exit
        {
            ...
            if (tmp != null) {
                ...
                !itemStack.getBaseStack().isEmpty()) {
                    int[] oreIds = OreDictionary.getOreIDs(itemStack.getBaseStack());
