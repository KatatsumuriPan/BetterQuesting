...
// TODO: Add ability for expansions to register modules for identifying and providing custom editors
// to various NBT data sets (inventory, tinker tool, magic, etc.)
...
public class PanelScrollingNBT extends CanvasScrolling implements IPEventListener {
    ...
    // TODO: Set a special tag that tells the editor about special serialised objects
    ...
    private PanelScrollingNBT(IGuiRect rect, int btnEdit, int btnAdv, int btnInsert, int btnDelete) {
        ...
        // Listens to its own buttons to update NBT values.
        // The parent screen defines what the IDs are and any furter actions
        PEventBroadcaster.INSTANCE.register(this, PEventButton.class);
        Keyboard.enableRepeatEvents(true);
    }

    // TODO: Implement this with a proper way of displaying and choosing between options
    // (and setting priority for default)
    /*
    ...
    private void refreshList() {
        ...
        if (nbt.getId() == 10) // NBTTagCompound
        {
            ...
            while (keys.hasNext()) {
                ...
                if (entry.getId() == 10) // Object
                {
                    PanelButtonStorage<String> btn = new PanelButtonStorage<>(
                            new GuiRectangle(lw, i * 16, rw - 48, 16, 0), btnEdit,
                ...
                } else if (entry.getId() == 8) // Text
                {
                    PanelTextField<String> text = new PanelTextField<>(new GuiRectangle(lw, i * 16, rw - 48, 16, 0),
                            "" + ((NBTTagString) entry).getString(), FieldFilterString.INSTANCE);
                    text.setCallback(new CallbackNBTTagString(tag, k)).setMaxLength(Integer.MAX_VALUE);
                    this.addPanel(text);

                    PanelButtonStorage<String> btn = new PanelButtonStorage<>(
                            new GuiRectangle(width - 48, i * 16, 16, 16, 0), btnEdit, "Aa", k);
                    this.addPanel(btn);
                } else if (entry.getId() == 1) // Byte/Boolean
                {
                    PanelTextField<Byte> text = new PanelTextField<>(new GuiRectangle(lw, i * 16, rw / 2, 16, 0),
                            "" + ((NBTPrimitive) entry).getByte(), FieldFilterNumber.BYTE);
                    text.setMaxLength(Integer.MAX_VALUE); // Put callback here
                    this.addPanel(text);

                    PanelButtonStorage<String> btn = new PanelButtonStorage<>(
                            new GuiRectangle(lw + rw / 2, i * 16, (int) Math.ceil(rw / 2F) - 32, 16, 0), btnEdit,
                            ((NBTPrimitive) entry).getByte() > 0 ? "true" : "false", k);
                    this.addPanel(btn);
                    ...
                    switch (entry.getId()) {
                        case 2: // Short
                        {
                            PanelTextField<Short> text = new PanelTextField<>(
                                    ...
                                    "" + ((NBTPrimitive) entry).getShort(), FieldFilterNumber.SHORT);
                        ...
                        case 3: // Integer
                        {
                            PanelTextField<Integer> text = new PanelTextField<>(
                                    new GuiRectangle(lw, i * 16, rw - 32, 16, 0), "" + ((NBTPrimitive) entry).getInt(),
                        ...
                        case 4: // Long
                        {
                            PanelTextField<Long> text = new PanelTextField<>(
                                    new GuiRectangle(lw, i * 16, rw - 32, 16, 0), "" + ((NBTPrimitive) entry).getLong(),
                        ...
                        case 5: // Float
                        {
                            PanelTextField<Float> text = new PanelTextField<>(
                                    new GuiRectangle(lw, i * 16, rw - 32, 16, 0),
                                    "" + ((NBTPrimitive) entry).getFloat(), FieldFilterNumber.FLOAT);
                        ...
                        case 6: // Double
                        {
                            PanelTextField<Double> text = new PanelTextField<>(
                                    new GuiRectangle(lw, i * 16, rw - 32, 16, 0),
                                    "" + ((NBTPrimitive) entry).getDouble(), FieldFilterNumber.DOUBLE);
                ...
                PanelButtonStorage<String> btnI = new PanelButtonStorage<>(
                        new GuiRectangle(width - 32, i * 16, 16, 16, 0), btnInsert, "+", k);
                ...
                PanelButtonStorage<String> btnD = new PanelButtonStorage<>(
                        new GuiRectangle(width - 16, i * 16, 16, 16, 0), btnDelete, "x", k);
            ...
            PanelButtonStorage<String> btnI = new PanelButtonStorage<>(new GuiRectangle(width - 32, i * 16, 16, 16, 0),
                    btnInsert, "+", "");
            btnI.setTextHighlight(new GuiColorStatic(128, 128, 128, 255), new GuiColorStatic(0, 255, 0, 255),
                    new GuiColorStatic(0, 255, 0, 255));
            this.addPanel(btnI);
        } else if (nbt.getId() == 9) // NBTTagList
        {
            ...
            for (; i < list.tagCount(); i++) {
                ...
                } else if (entry.getId() == 8) // Text
                {
                    ...
                    PanelButtonStorage<Integer> btn = new PanelButtonStorage<>(
                            new GuiRectangle(width - 48, i * 16, 16, 16, 0), btnEdit, "Aa", i);
                    this.addPanel(btn);
                } else if (entry.getId() == 1) // Byte/Boolean
                {
                    PanelTextField<Byte> text = new PanelTextField<>(new GuiRectangle(lw, i * 16, rw / 2, 16, 0),
                    ...
                    PanelButtonStorage<Integer> btn = new PanelButtonStorage<>(
                            new GuiRectangle(lw + rw / 2, i * 16, (int) Math.ceil(rw / 2F) - 32, 16, 0), btnEdit,
                            ((NBTPrimitive) entry).getByte() > 0 ? "true" : "false", i);
                    this.addPanel(btn);
                ...
                PanelButtonStorage<Integer> btnI = new PanelButtonStorage<>(
                        new GuiRectangle(width - 32, i * 16, 16, 16, 0), btnInsert, "+", i);
                btnI.setTextHighlight(new GuiColorStatic(128, 128, 128, 255), new GuiColorStatic(0, 255, 0, 255),
                        new GuiColorStatic(0, 255, 0, 255));
                this.addPanel(btnI);

                PanelButtonStorage<Integer> btnD = new PanelButtonStorage<>(
                        new GuiRectangle(width - 16, i * 16, 16, 16, 0), btnDelete, "x", i);
                btnD.setTextHighlight(new GuiColorStatic(128, 128, 128, 255), new GuiColorStatic(255, 0, 0, 255),
                        new GuiColorStatic(255, 0, 0, 255));
                this.addPanel(btnD);
            }
            ...
            PanelButtonStorage<Integer> btnI = new PanelButtonStorage<>(new GuiRectangle(width - 32, i * 16, 16, 16, 0),
                    btnInsert, "+", i);
            btnI.setTextHighlight(new GuiColorStatic(128, 128, 128, 255), new GuiColorStatic(0, 255, 0, 255),
                    new GuiColorStatic(0, 255, 0, 255));
            this.addPanel(btnI);
    ...
    private void onButtonPress(PEventButton event) {
        ...
        } else if (btn.getButtonID() == btnAdv) // Open advanced editor (on supported types)
        {
            if (entry.getId() == 10) {
                ...
                // Not currently available but will be when context list editors (enchantments/inventories/etc) are
                // available
            ...
            if (nbt.getId() == 10) {
                ...
                mc.displayGuiScreen(new GuiNbtAdd(mc.currentScreen, (NBTTagList) nbt,
                        ((PanelButtonStorage<Integer>) btn).getStoredValue()));
            }
