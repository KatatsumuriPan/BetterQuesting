...
public class GuiTaskEditor extends GuiScreenCanvas implements IPEventListener, IVolatileScreen, INeedsRefresh {
    ...
    public void initPanel() {
        ...
        CanvasSearch<IFactoryData<ITask, NBTTagCompound>, IFactoryData<ITask, NBTTagCompound>> cvRegSearch = new CanvasSearch<IFactoryData<ITask, NBTTagCompound>, IFactoryData<ITask, NBTTagCompound>>(
            ...
            @Override
            protected void queryMatches(IFactoryData<ITask, NBTTagCompound> value, String query,
                                        ArrayDeque<IFactoryData<ITask, NBTTagCompound>> results) {
        ...
        PanelVScrollBar scTsk = new PanelVScrollBar(
    ...
    private void onButtonPress(PEventButton event) {
        ...
        {
            IFactoryData<ITask, NBTTagCompound> fact = ((PanelButtonStorage<IFactoryData<ITask, NBTTagCompound>>) btn)
                    .getStoredValue();
    ...
    private void refreshTasks() {
        ...
        for (int i = 0; i < dbTsk.size(); i++) {
            ...
            qtList.addPanel(new PanelButtonStorage<>(new GuiRectangle(0, i * 16, w - 16, 16, 0), 3,
                    QuestTranslation.translate(task.getUnlocalisedName()), task));
            qtList.addPanel(new PanelButtonStorage<>(new GuiRectangle(w - 16, i * 16, 16, 16, 0), 2,
                    "" + TextFormatting.RED + TextFormatting.BOLD + "x", task));
        }
    }
