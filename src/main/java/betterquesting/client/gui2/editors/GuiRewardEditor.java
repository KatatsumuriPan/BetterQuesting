...
public class GuiRewardEditor extends GuiScreenCanvas implements IPEventListener, IVolatileScreen, INeedsRefresh {
    ...
    private void refreshRewards() {
        ...
        for (int i = 0; i < dbRew.size(); i++) {
            ...
            qrList.addPanel(new PanelButtonStorage<>(new GuiRectangle(0, i * 16, w - 16, 16, 0), 3,
                    QuestTranslation.translate(reward.getUnlocalisedName()), reward));
            qrList.addPanel(new PanelButtonStorage<>(new GuiRectangle(w - 16, i * 16, 16, 16, 0), 2,
                    "" + TextFormatting.RED + TextFormatting.BOLD + "x", reward));
        }
    }
