...
public class GuiHome extends GuiScreenCanvas {
    ...
    public void initPanel() {
        ...
        CanvasTextured splashCan = new CanvasTextured(
                new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(0, 0, 0, 32), 0), homeSplashBG);
        inCan.addPanel(splashCan);
        CanvasTextured splashTitle = new CanvasTextured(new GuiTransform(new Vector4f(ancX, ancY, ancX, ancY),
                new GuiPadding(offX, offY, -256 - offX, -128 - offY), 0), homeSplashTitle);
        splashCan.addPanel(splashTitle);

        PanelButton btnExit = new PanelButton(
                new GuiTransform(new Vector4f(0F, 1F, 0.25F, 1F), new GuiPadding(0, -32, 0, 0), 0), 0,
                QuestTranslation.translate("betterquesting.home.exit"));
        inCan.addPanel(btnExit);

        PanelButton btnQuests = new PanelButton(
                new GuiTransform(new Vector4f(0.25F, 1F, 0.5F, 1F), new GuiPadding(0, -32, 0, 0), 0), 1,
                QuestTranslation.translate("betterquesting.home.quests"));
        inCan.addPanel(btnQuests);
        PanelButton btnParty = new PanelButton(
                new GuiTransform(new Vector4f(0.5F, 1F, 0.75F, 1F), new GuiPadding(0, -32, 0, 0), 0), 2,
        ...
        PanelButton btnTheme = new PanelButton(
                new GuiTransform(new Vector4f(0.75F, 1F, 1F, 1F), new GuiPadding(0, -32, 0, 0), 0), 3,
                QuestTranslation.translate("betterquesting.home.theme"));
        inCan.addPanel(btnTheme);

        PanelButton btnNotif = new PanelButton(new GuiTransform(GuiAlign.BOTTOM_RIGHT, -140, -52, 136, 16, 0), 420,
            ...
            public void onButtonClick() {
                ...
                if (betterquesting.handlers.ConfigHandler.config != null) {
                    betterquesting.handlers.ConfigHandler.config
                            .get(Configuration.CATEGORY_GENERAL, "Quest Notices", true).set(BQ_Settings.questNotices);
        ...
        btnNotif.setTooltip(
                Collections.singletonList(QuestTranslation.translate("betterquesting.notification.tooltip")));
        ...
        if (Minecraft.getMinecraft().isIntegratedServerRunning() && SaveLoadHandler.INSTANCE.hasUpdate()) {
            ...
            tstBtn.setTooltip(Collections
                    .singletonList(QuestTranslation.translateTrimmed("betterquesting.tooltip.update_quests", true)));
    ...
    private void onButtonPress(PanelEvent event) {
        ...
        {
            mc.displayGuiScreen(
                    new GuiNbtEditor(this, QuestSettings.INSTANCE.writeToNBT(new NBTTagCompound()), (value) -> {
        ...
        } else if (btn.getButtonID() == 5) // Update me
        {
            ...
            if (qFile.exists()) {
                FMLCommonHandler.instance().getMinecraftServerInstance().addScheduledTask(() -> {
                    ...
                    NBTTagCompound j1 = NBTConverter.JSONtoNBT_Object(JsonHelper.ReadFromFile(qFile),
