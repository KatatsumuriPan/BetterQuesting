...
public class PanelTaskInteractEntity extends CanvasMinimum {
    ...
    public void initPanel() {
        ...
        PanelItemSlot itemSlot = new PanelItemSlot(new GuiTransform(GuiAlign.TOP_LEFT, 0, 0, 32, 32, 0), -1,
                task.targetItem, false, true);
        this.addPanel(itemSlot);

        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 32, 8, 16, 16, 0),
        ...
        this.addPanel(
                new PanelTextBox(new GuiTransform(GuiAlign.TOP_LEFT, 0, 34, 32, 14, 0), prog + "/" + task.required)
                        .setAlignment(1).setColor(PresetColor.TEXT_MAIN.getColor()));

        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 0, 48, 24, 24, 0),
                BQSTextures.HAND_LEFT.getTexture()));
        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 24, 48, 24, 24, 0),
                BQSTextures.HAND_RIGHT.getTexture()));
        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 0, 72, 24, 24, 0),
                BQSTextures.ATK_SYMB.getTexture()));
        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 24, 72, 24, 24, 0),
        ...
        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 16, 64, 8, 8, 0),
                task.useOffHand ? txTick : txCross));
        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 40, 64, 8, 8, 0),
                task.useMainHand ? txTick : txCross));
        this.addPanel(
                new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 16, 88, 8, 8, 0), task.onHit ? txTick : txCross));
        this.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_LEFT, 40, 88, 8, 8, 0),
