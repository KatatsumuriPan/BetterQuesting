...
public class GuiPartyCreate extends GuiScreenCanvas implements IPEventListener, INeedsRefresh {
    ...
    public void initPanel() {
        ...
        CanvasEmpty cvLeftHalf = new CanvasEmpty(
                new GuiTransform(GuiAlign.HALF_LEFT, new GuiPadding(16, 32, 8, 32), 0));
        cvBackground.addPanel(cvLeftHalf);

        PanelPlayerPortrait pnPortrait = new PanelPlayerPortrait(
                new GuiTransform(GuiAlign.TOP_CENTER, -32, 0, 64, 64, 0), mc.player).setDepth(-16F);
        ...
        cvLeftHalf.addPanel(new PanelGeneric(new GuiTransform(GuiAlign.TOP_CENTER, 16, 48, 24, 24, 0),
                new ItemTexture(new BigItemStack(BetterQuesting.extraLife,
                        LifeDatabase.INSTANCE.getLives(QuestingAPI.getQuestingUUID(mc.player))), true, true)
                                .setDepth(32F)));

        PanelTextBox txName = new PanelTextBox(
                new GuiTransform(GuiAlign.BOTTOM_EDGE, new GuiPadding(16, -44, 16, 28), 0),
        ...
        flName = new PanelTextField<>(new GuiTransform(GuiAlign.BOTTOM_EDGE, new GuiPadding(16, -32, 16, 16), 0),
                "New Party", FieldFilterString.INSTANCE);
        cvLeftHalf.addPanel(flName);

        PanelButton btnCreate = new PanelButton(
                new GuiTransform(GuiAlign.BOTTOM_EDGE, new GuiPadding(16, -16, 16, 0), 0), 1,
                QuestTranslation.translate("betterquesting.btn.party_new"));
        cvLeftHalf.addPanel(btnCreate);

        CanvasEmpty cvRightHalf = new CanvasEmpty(
    ...
    private void refreshInvites() {
        ...
        for (int i = 0; i < invites.size(); i++) {
            ...
            PanelButtonStorage<Integer> btnJoin = new PanelButtonStorage<>(
                    new GuiRectangle(cvWidth - 50, i * 24, 50, 16, 0), 2,
                    QuestTranslation.translate("betterquesting.btn.party_join"), pid);
            invitePanel.addPanel(btnJoin);

            String pName = party == null ? "Unknown (" + pid + ")" :
            ...
            ValueFuncIO<Float> expFunc = new ValueFuncIO<>(
                    () -> (float) ((exp - System.currentTimeMillis()) / 300000D));
            flExpiry.setFillDriver(expFunc);
            flExpiry.setFillColor(new GuiColorTransition(new GuiColorStatic(0xFF00FF00),
                    new GuiColorPulse(0xFFFF0000, 0xFFC00000, 1F, 0F)).setupBlending(false, 0.2F)
