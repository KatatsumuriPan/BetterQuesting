...
public class GuiPartyManage extends GuiScreenCanvas implements IPEventListener, INeedsRefresh {
    ...
    public void initPanel() {
        ...
        if (status == null)
            status = EnumPartyStatus.MEMBER; // Fallback (potentially exploitable I know)
        ...
        PanelTextBox txTitle = new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 16, 0, -32), 0),
                                                QuestTranslation.translate("betterquesting.title.party", party.getProperties().getProperty(NativeProps.NAME)))
                .setAlignment(1);
        ...
        PanelButtonStorage<String> btnLeave = new PanelButtonStorage<>(new GuiTransform(GuiAlign.MID_CENTER, -75, 32, 70, 16, 0),
                                                                       3,
                                                                       QuestTranslation.translate("betterquesting.btn.party_leave"),
                                                                       mc.player.getGameProfile().getName());
        cvLeftHalf.addPanel(btnLeave);

        PanelButton btnInvite = new PanelButton(new GuiTransform(GuiAlign.MID_CENTER, 5, 32, 70, 16, 0),
                                                2,
        ...
        for (int i = 0; i < partyMemList.size(); i++) {
            ...
            PanelButtonStorage<String> btnKick = new PanelButtonStorage<>(new GuiRectangle(cvWidth - 32, i * 32, 32, 32, 0),
                                                                          3,
