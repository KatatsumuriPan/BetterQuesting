...
public class GuiQuestHelp extends GuiScreenCanvas {
    ...
    public void initPanel() {
        ...
        PanelButton btnBack = new PanelButton(new GuiTransform(GuiAlign.BOTTOM_CENTER, -100, -16, 200, 16, 0), 0,
        ...
        CanvasScrolling cvTopics = new CanvasScrolling(
                new GuiTransform(GuiAlign.LEFT_EDGE, new GuiPadding(16, 32, -108, 24), 0));
        cvBackground.addPanel(cvTopics);

        PanelVScrollBar scTopic = new PanelVScrollBar(
                new GuiTransform(GuiAlign.LEFT_EDGE, new GuiPadding(108, 32, -116, 24), 0));
        cvBackground.addPanel(scTopic);
        cvTopics.setScrollDriverY(scTopic);

        CanvasScrolling cvDesc = new CanvasScrolling(
                new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(124, 48, 24, 24), 0)).enableBlocking(false);
        cvBackground.addPanel(cvDesc);

        PanelVScrollBar scDesc = new PanelVScrollBar(
                new GuiTransform(GuiAlign.RIGHT_EDGE, new GuiPadding(-24, 48, 16, 24), 0));
        cvBackground.addPanel(scDesc);
        cvDesc.setScrollDriverY(scDesc);

        PanelTextBox txtTitle = new PanelTextBox(
                new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(124, 36, 16, -48), 0), "").setAlignment(1);
        txtTitle.setColor(PresetColor.TEXT_HEADER.getColor());
        cvBackground.addPanel(txtTitle);

        PanelTextBox txtDesc = new PanelTextBox(new GuiRectangle(0, 0, cvDesc.getTransform().getWidth(), 16, 0), "",
                true);
        ...
        for (int i = 0; i < topics.length; i++) {
            ...
            cvTopics.addPanel(new PanelButtonStorage<>(new GuiRectangle(0, i * 16, width, 16, 0), -1, ht.getTitle(), ht)
                    .setCallback(value -> {
