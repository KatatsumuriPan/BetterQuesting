...
public class FtbqTaskXP {
    ...
    /*
     * Fun Fact:
     * As of writing this, FTBQ doesn't care if the level you submit to its task is from level 30 or level 1.
     * This means high level players get screwed over. GG
     */
    public ITask[] convertTask(NBTTagCompound tag) {
        TaskXP task = new TaskXP();
        // FTBQ is consume only. Not really sure why it doesn't allow detect
        task.consume = true;
        // Suuuure FTBQ. The vanilla int XP level can totally exceed 2.14B
        task.amount = (int) (tag.getLong("value") % Integer.MAX_VALUE);
