...
public class FactoryPolyTextureC implements IFactoryData<IGuiTexture, JsonObject> {
    ...
    public IGuiTexture loadFromData(JsonObject data) {
        ...
        try {
            color = QuestingAPI.getAPI(ApiReference.RESOURCE_REG)
                    .getColorReg()
        ...
        try {
            borColor = QuestingAPI.getAPI(ApiReference.RESOURCE_REG)
                    .getColorReg()
                    .createNew(new ResourceLocation(JsonHelper.GetString(jCol, "colorType", "null")), jCol);
            if (borColor == null)
    ...
    @Override
    public ResourceLocation getRegistryName() { return ID_NAME; }
