...
public class FactorySlicedTexture implements IFactoryData<IGuiTexture, JsonObject> {
    ...
    public SlicedTexture loadFromData(JsonObject data) {
        ResourceLocation atlas = new ResourceLocation(
                JsonHelper.GetString(data, "atlas", PresetTexture.TX_NULL.toString()));
        int sliceMode = MathHelper.clamp(JsonHelper.GetNumber(data, "sliceMode", 0).intValue(), 0,
        ...
        return new SlicedTexture(atlas, new GuiRectangle(bounds[0], bounds[1], bounds[2], bounds[3]),
