...
public abstract class CanvasQuestDatabase extends CanvasSearch<DBEntry<IQuest>, DBEntry<IQuest>> {
    ...
    @Override
    protected Iterator<DBEntry<IQuest>> getIterator() { return QuestDatabase.INSTANCE.getEntries().iterator(); }

    @Override
    protected void queryMatches(DBEntry<IQuest> entry, String query, final ArrayDeque<DBEntry<IQuest>> results) {
        if (("" + entry.getID()).contains(query) || entry.getValue().getProperty(NativeProps.NAME).toLowerCase().contains(query) || QuestTranslation.translate(
                                                                                                                                                               entry.getValue()
